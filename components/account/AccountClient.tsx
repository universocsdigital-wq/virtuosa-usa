      ...
      <div className="w-full max-w-[820px] border border-[#D9C8B5] bg-white/55 p-7 shadow-[0_20px_60px_rgba(42,23,18,0.09)] sm:p-10 lg:px-14">
        ...
        {step === "email" ? (
          <form onSubmit={requestCode} className="mt-7">
            ...
            <input id="account-email" type="email" required value={email} onChange={(event) => setEmail(event.target.value)} className="h-12 w-full border border-[#CDB89F] bg-[#FFFDF8] px-4 font-sans text-sm text-[#2A1712] outline-none focus:border-[#8A5A36]" autoComplete="email" />
            <button type="submit" disabled={loading} className="mt-4 flex min-h-12 w-full items-center justify-center bg-[#8A5A36] px-6 font-sans text-[11px] font-bold uppercase tracking-[0.14em] text-white shadow-[0_8px_20px_rgba(42,23,18,0.15)] disabled:opacity-60">{loading ? "Enviando..." : "Receber código \u2192"}</button>
          </form>
        ) : (
          <form onSubmit={verifyCode} className="mt-7">
            ...
            <input id="account-code" inputMode="numeric" pattern="[0-9]*" required value={token} onChange={(event) => setToken(event.target.value.replace(/\D/g, "").slice(0, 8))} className="h-12 w-full border border-[#CDB89F] bg-[#FFFDF8] px-4 text-center font-sans text-xl tracking-[0.35em] text-[#2A1712] outline-none focus:border-[#8A5A36]" autoComplete="one-time-code" />
            <button type="submit" disabled={loading} className="mt-4 flex min-h-12 w-full items-center justify-center bg-[#8A5A36] px-6 font-sans text-[11px] font-bold uppercase tracking-[0.14em] text-white shadow-[0_8px_20px_rgba(42,23,18,0.15)] disabled:opacity-60">{loading ? "Entrando..." : "Consultar pedidos \u2192"}</button>
